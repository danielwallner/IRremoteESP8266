...
const uint16_t kBeoFreq = 455;  // kHz.
                                ...
                                const bool backToBack)
{
    ...
    enableIROut(kBeoFreq, datalink ? kDutyMax : kDutyDefault);

#ifdef BANG_OLUFSEN_CHECK_MODULATION
    // Don't send, just measure during 1 ms
    unsigned long startTime = micros();
    uint16_t testTime = 10000;
    uint32_t pulses = mark(testTime);
    uint32_t timespent = micros() - startTime;
    uint32_t frequency = pulses * (1000000 / testTime);
